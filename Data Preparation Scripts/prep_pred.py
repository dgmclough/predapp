...
profiles = "Source Data/live_profiles.csv"
...
def ratios(df, f, x ,y):
    alt_profiles = "Source Data/alt_profiles.csv"
    ...
    profiles.to_csv("Source Data/alt_profiles.csv", sep =',', index = False) 
	profiles.to_csv("Predatour/tmp/alt_profiles.csv", sep =',', index = False)  
    return df
...
def max_min(df):
    ...
    max_min.to_csv("Source Data/max_min_values.csv", sep =',', index = False)
    ...
    std = pd.DataFrame(ms)
    std.to_csv("Source Data/std_values.csv", sep =',', index = False)
...
def prep():
    df = create_dataset("Source Data/201*.xlsx")
    ...
    df = rearrange(df)
    max_min(df)
    df.to_csv("Source Data/df.csv", sep =',', index = False)
    df1 = df
    df = normalize(df)
    df = rearrange(df)
    df.to_csv("Source Data/df_norm.csv", sep =',', index = False)
    df = standardize(df)
    df = rearrange(df)
    df.to_csv("Source Data/df_stand.csv", sep =',', index = False)
    return df1
